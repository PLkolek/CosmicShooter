...
BoundingBoxComponent::BoundingBoxComponent(int EID) :
		Component(EID), boundingBox(), topSolid(false), rightSolid(false), bottomSolid(
				false), leftSolid(false)
{
}
BoundingBoxComponent::BoundingBoxComponent(sf::Rect<double> boundingBox,
		int EID) :
...
BoundingBoxComponent::BoundingBoxComponent(rapidxml::xml_node<>* componentNode) :
		Component(-1), topSolid(false), rightSolid(false), bottomSolid(false), leftSolid(
				false)
{
	read(componentNode);
}
...
void BoundingBoxComponent::read(rapidxml::xml_node<>* componentNode)
{
	...
	if (componentNode->first_attribute("topSolid") != 0)
		topSolid = boost::lexical_cast<bool>(
				componentNode->first_attribute("topSolid")->value());
	if (componentNode->first_attribute("rightSolid") != 0)
		rightSolid = boost::lexical_cast<bool>(
				componentNode->first_attribute("rightSolid")->value());
	if (componentNode->first_attribute("bottomSolid") != 0)
		bottomSolid = boost::lexical_cast<bool>(
				componentNode->first_attribute("bottomSolid")->value());
	if (componentNode->first_attribute("leftSolid") != 0)
		leftSolid = boost::lexical_cast<bool>(
				componentNode->first_attribute("leftSolid")->value());

	for (rapidxml::xml_attribute<> *att = componentNode->first_attribute(
			"collisionGroup"); att; att = att->next_attribute("collisionGroup"))
	{
